...
| `locationKey` | 可选 | `weathercn:` + 城市代码，如 `weathercn:<城市代码>`。**镇级代码实测返回空 `{}`** |
...
| 方式 | 结果 |
|---|---|
| `locationKey=weathercn:<县代码+3位镇序号>`（镇级） | ❌ 返回 `{}`，**镇级代码不可用** |
| `locationKey=weathercn:<县代码>`（县级） | ✅ 可用 |
| 省略 locationKey + `isLocated=true` + 真实坐标 | ✅ 可用，**坐标反查归到最近县级站点** |

- 响应中 `sourceMaps` 字段可确认数据源，如 `{"current":{"temperature":"weatherbj(locationKey=<县代码>)"}}` 表示数据来自该县站点。
- 精度上限为**区县级**（weatherbj 站点）；`isLocated` 只是省去手动查代码，粒度不变。
- 不同坐标对照实测：数据随坐标明显变化（气压/天气码不同）——确认按坐标就近取数。

## 3. 请求头要求（实测）

- **设备直连无需 UA/Referer**，裸请求即返回 200。
...
- **weathercn 城市代码结构**：`<省区代码><市代码><县代码>`，县代码 + 3 位序号为乡镇代码（中国天气网 town 页面可用，**小米接口不可用**）。
- 需要查自己位置的县级代码时，可在中国天气网（weather.com.cn）搜索当地天气，URL 中的数字即代码。
